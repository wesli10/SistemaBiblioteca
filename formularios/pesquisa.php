...
while ($linha = mysqli_fetch_assoc($dados)) {
    ...
    $autor = $linha['autor'];
    $tema = $linha['tema'];
    $data_publi = $linha['data_publi'];
    $categoria = $linha['categoria'];
    $qtd_copias = $linha['qtd_copias'];
    echo "<tr>
                    <th scope='row'>$nome_livro</th>
                    <td>$autor</td>
                    <td>$tema</td>
                    <td>$data_publi</td>
                    <td>$categoria</td>
                    <td>$qtd_copias</td>
                    <td width=150px> <a href='/formularios/cadastro_livro_edit.php?id=$id_livro' class='btn btn-success'>Editar</a>
                        <a href='#' class='btn btn-danger btn-sm' data-bs-toggle='modal' data-bs-target='#confirma' onclick=" . '"' . "get_data($id_livro, '$nome')" . '"' . ">Excluir</a>
...
?>
    ...
    <div class="modal fade" id="confirma" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            ...
            <form action='/scripts/excluir_script.php' method='POST'>
